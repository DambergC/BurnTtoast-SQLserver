...
function Invoke-Registration {
    ...
    foreach($g in $config.ClientGroups){Invoke-ToastSql $conn -SqlCredential $sqlCredential $sql @{ComputerName=$computer;GroupName=$g} -CommandTimeoutSeconds $config.CommandTimeoutSeconds -NonQuery}
}
if($Register){Invoke-Registration;Write-Output "Registered $computer";if($Once){return}}
function Invoke-Poll {
    $rows=Invoke-ToastSql $conn -SqlCredential $sqlCredential 'EXEC dbo.usp_GetPendingToast @ComputerName' @{ComputerName=$computer} -CommandTimeoutSeconds $config.CommandTimeoutSeconds
    foreach($row in $rows){try{New-BurntToastNotification -Text @($row.Title,$row.Body);Invoke-ToastSql $conn -SqlCredential $sqlCredential 'EXEC dbo.usp_RecordToastDelivery @ComputerName,@MessageId,@Status,@ErrorMessage' @{ComputerName=$computer;MessageId=$row.MessageId;Status='Delivered';ErrorMessage=$null} -CommandTimeoutSeconds $config.CommandTimeoutSeconds -NonQuery}catch{Invoke-ToastSql $conn -SqlCredential $sqlCredential 'EXEC dbo.usp_RecordToastDelivery @ComputerName,@MessageId,@Status,@ErrorMessage' @{ComputerName=$computer;MessageId=$row.MessageId;Status='Failed';ErrorMessage=$_.Exception.Message} -CommandTimeoutSeconds $config.CommandTimeoutSeconds -NonQuery}}
}
